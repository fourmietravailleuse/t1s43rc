...
// FIL D'ARIANE 
// CREE PAR DANIEL ROCH (SEOMIX)
// MODIFIE PAR CREASIT

function creasit_content_get_category_parents($id, $link = false,$separator = '/',$nicename = false,$visited = array()) {
	$final = '';
	...
	if ($parent->parent && ($parent->parent != $parent->term_id ) && !in_array($parent->parent, $visited)) {
		...
		$final .= creasit_content_get_category_parents( $parent->parent, $link, $separator, $nicename, $visited );
	}
	if ($link)
...
// Breadcrumb
function creasit_content_breadcrumb() {
	// Global vars
	global $wp_query;
	...
	$final .= creasit_content_get_category_parents($category_id,TRUE,$sep).$data."<a href='$permalink' rel='v:url' property='v:title' title='$title'>$title</a>".$dataend.$sep.the_title('','',FALSE);}
	// Post type
	...
	$category_id = get_cat_ID( $category[0]->cat_name );
	if ($category_id != 0)
	  $final .= creasit_content_get_category_parents($category_id,TRUE,$sep);
	...
	if ($category->parent != 0) 
	  $final .= creasit_content_get_category_parents($categoryparent, true, $sep, true);
	if ( $paged <= 1 )
